...
for line in file :
    ...
    try:
        ...
        if ping_reply == 0:
            ...
            connection.send("\n")
            time.sleep(1)
            # Open a config user selected file for reading
            config_file = open('ssg-template.txt', 'r')
            # Starting from the beginning of the file
            config_file.seek(0)
            # Writing each line in the file to the device and send each line with and enter \n with a timedelay of 2 seconds between each commands as a safety margin of screen scrapping 
